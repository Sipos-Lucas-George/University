...
std::vector<int> Graph::recursiveBFS(std::vector<int> finalPath, std::vector<int> randomPath, std::set<int> visited, int start, int index)
{
    visited.insert(index);
    if (this->outboundMap.size() == 0)
        return randomPath;
    for (int i = 0; i < this->outboundMap[index].size(); ++i) {
        if (visited.count(i) != 0) {
            if (i + 1 == this->outboundMap[index].size())
                return randomPath;
            continue;
        }
        randomPath.push_back(this->outboundMap[index][i]);
        if (randomPath.back() == start) {
            if (randomPath.size() < finalPath.size()) {
                finalPath = randomPath;
            }
            randomPath.pop_back();
            return randomPath;
        }
        randomPath = this->recursiveBFS(finalPath, randomPath, visited, start, this->outboundMap[index][i]);
        randomPath.pop_back();
    }
    return finalPath;
}

std::vector<int> Graph::backwardBFS(int end, int start)
{
    ...
    if (end == start)
        return randomPath;
    return this->recursiveBFS(finalPath, randomPath, visited, start, end);
}
