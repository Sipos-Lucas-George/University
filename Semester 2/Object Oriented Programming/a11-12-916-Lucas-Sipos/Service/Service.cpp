...
void Service::incrementLikes(int pos) {
    ...
    this->watchList->incrementLikes(pos);
}
...
int Service::getIndexOfMovie(const Movie& movie) {
    int i = 0;
    for (const auto& m : this->getAllRepository()){
        if (m.getTitle() == movie.getTitle() && m.getGenre() == movie.getGenre() && m.getYear() == movie.getYear())
            return i;
        ++i;
    }
    return -1;
