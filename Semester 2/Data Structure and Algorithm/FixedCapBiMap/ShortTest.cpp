...
void testAll() { //call each function to see if it is implemented
	...
	// new functionality
	std::pair<TValue, TValue> ans4(111, 1);
	std::pair<TValue, TValue> ans5(5, NULL_TVALUE);
	assert(m.add(1, 1) == true); 
	assert(m.removeKey(1231231) == ans3);
	assert(m.removeKey(1) == ans4);
	assert(m.removeKey(5) == ans5);
	assert(m.size() == 4);
}
