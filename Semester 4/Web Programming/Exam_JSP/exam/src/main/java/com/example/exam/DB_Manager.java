...
public class DB_Manager {
    ...
    public void addArticles(String name, int journal) {

        String statement = String.format("insert into Article (journal_id, user, summary, date) values (%d, '%s', 'summary', 1234)", journal, name);
        try {
            var connection = DriverManager.getConnection(connection_url, DB_Manager.name, password);
            var preparedStatement = connection.prepareStatement(statement);
