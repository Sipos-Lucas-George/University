...
def delete_student_by_id(students, id):
    find_student_id = find_by_id(students, id)
    if find_student_id is None:
        raise ValueError(f"Student with id = {id} does not exist!")
    students.remove(find_student_id)


# ==================== UI =====================


def ui_delete_student_by_id(student, id):
    try:
        delete_student_by_id(student, int(id))
...
def test_delete_student_by_id():
    students = set_up()
    ui_delete_student_by_id(students, 1)
    assert len(students) == 1
    assert students == [{'id': 2, 'name': 'Radu', 'grade': 2}]
    try:
