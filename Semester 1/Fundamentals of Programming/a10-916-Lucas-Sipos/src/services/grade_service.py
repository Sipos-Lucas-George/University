...
class GradeService:
    ...
    def getTopDisciplines(self, disciplineService):
        ...
        lastDisciplineID = listOfGrades[0].disciplineID
        for i in listOfGrades:
            if i.disciplineID == lastDisciplineID:
                sumOfGrades += i.name
                numberOfGrades += 1
            else:
        ...
        completeListOfFailingStudents = list()
        for i in listOfTopAverageAtDisciplines:
            completeListOfFailingStudents.append([disciplineService.getDisciplineFromDictionary(i[1]), i[0]])
        return completeListOfFailingStudents
