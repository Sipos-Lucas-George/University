...
create or alter procedure connectTableToTest (@tableName varchar(50), @testName varchar(50), @rows int, @pos int) as
    ...
    if exists(select * from TestTables T1 join Tests T2 on T1.testID = T2.testID where T2.Name = @testName and position = @pos) begin
...
create or alter procedure runTest (@testName varchar(50)) as
    ...
    while @@FETCH_STATUS = 0 begin
        ...
        values
            (@testRunId, (select TableID from Tables where Name=@table), @startTime, @endTime)
        fetch tableCursor into @table, @rows, @pos
    end
    close tableCursor
    ...
    while @@FETCH_STATUS = 0 begin
        ...
        values
            (@testRunId, (select ViewID from Views where Name=@view), @startTime, @endTime)
        fetch viewCursor into @view
    end
    close viewCursor
