...
public class ProgramState {
    ...
    public ProgramState(IStack<IStatement> execStack, IDictionary<String, Value> symTable, IList<String> out, IDictionary<String, BufferedReader> fileTable, IHeap heap, ISemaphore semaphore) {
    ...
    public String semToString(){
        StringBuilder builder = new StringBuilder();
        Map<Integer, Pair<Integer, List<Integer>>> semaphore = this.semaphore.getSemaphoreTable();
        for (Integer key : semaphore.keySet()){
            builder.append(key).append(" -> ").append(semaphore.get(key).first).append(": ").append(semaphore.get(key).second.toString()).append("\n");
        }
        return builder.toString();
    }
