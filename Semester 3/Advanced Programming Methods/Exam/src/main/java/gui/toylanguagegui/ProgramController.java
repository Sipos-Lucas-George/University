...
public class ProgramController {
    ...
    @FXML
    private TableColumn<Pair<Integer, Pair<Integer, List<Integer>>>, String> SemCol3;
    ...
    public void initialize() {
        ...
        SemCol2.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().second.first.toString()));
        SemCol3.setCellValueFactory(p -> new SimpleStringProperty(p.getValue().second.second.toString()));
    ...
    private void populateSem() {
        ...
        for (Map.Entry<Integer, Pair<Integer, List<Integer>>> entry : semaphore.getSemaphoreTable().entrySet())
            semList.add(new Pair<>(entry.getKey(), new Pair<>(entry.getValue().first, entry.getValue().second)));
        this.SemTable.setItems(FXCollections.observableList(semList));
